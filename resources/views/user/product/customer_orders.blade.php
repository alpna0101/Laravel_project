				  ...
				  <table class="table" colspacing="0" colpadding="0">
					    <thead>
					      <tr>
					        ...
					        <th>Send Payment Instruction</th>
					   ...
					   <?php  //print_r($orders);die; ?>
						...
						{{date('d/m/Y',strtotime($date1))}}</td>
					        ...
					        <td> @if($c->invoice_date!="0000-00-00 00:00:00")
                            <a href ="{{url('/view_invoice')}}/{{$c->id}}">#invoice{{$c->id}}</a>
                            @else
                            Invoice Is Not Available
                            @endif</td>
					        @if($c->invoice_date!="0000-00-00 00:00:00")
					         <td><button type="button" class="btn btn-default product_submit1 send_instruction"  data-id="{{$c->id}}">Resend </button></td>
					        @else
					        <td><button type="button" class="btn btn-default product_submit send_instruction"  data-id="{{$c->id}}">Send </button></td>
					        @endif
				   ...
				   <form id="checkoutForm" action="{{url('/transaction_update')}}" method="post"> 
                     	   ...
                     	   <div id="shippingModal" class="modal fade" role="dialog">
				             <form id="checkoutForm" action="{{url('/save_shipping')}}" method="post" enctype="multipart/form-data"> 
										  <div class="modal-dialog">
...
$('document').ready(function() {
	$("#ongoing").click(function(){
      var url = "{{url('customerorders')}}?action=ongoing";
      window.location.href = url;
	});
	$("#completed").click(function(){
      var url = "{{url('customerorders')}}?action=complete";
      window.location.href = url;
	});
	$("#pending").click(function(){
      var url = "{{url('customerorders')}}";
      window.location.href = url;
	});
