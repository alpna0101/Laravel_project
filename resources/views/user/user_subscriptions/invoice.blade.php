	...
	<div class="row content-row">
		...
		<div class="page-inner col-sm-9 col-md-10">
			...
			<div class="invoice">
				...
				<h3 class="no-margin text-center mb-20 mt-0">{{tr('invoice')}}</h3>	
						 		...
						 		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-6 invoice-img" style="background-image: url({{asset('images/invoice-bg.jpg')}});">
							 		<div class="invoice-overlay">
							 			<div class="invoice-desc">
										 	<h3 class="no-margin black-clr">{{$user_subscription_details->title}}</h3>
										 	<p class="invoice-desc"><?= $user_subscription_details->description ?></p>
									 	</div>
									</div>
								</div>
									...
									<div class="spacing1">
									 	<table  class="table text-right top-space table-sripped">
									 		<tbody>
											    ...
											    <tr class="active">
											        <td>{{tr('total')}}</td>
											        <td>{{Setting::get('currency')}} <span id="remaining_amount">{{$user_subscription_details->amount}}</span></td>
											    </tr> 
										    </tbody>
										</table>

										@if($user_subscription_details->amount > 0)

										<form method="post" action="">

											<!-- coupon code -->
											<div class="input-group coupon-code" style="display: none;">
									    	...
									    	<input type="hidden" name="channel_id" value="{{$user_subscription_details->channel_id}}">
											...
											@if(Setting::get('payment_type') == 'stripe')
												<div>
												    <label class="radio1">
													    <input id="radio2" type="radio" name="payment_type" value="{{CARD}}">
				 						...
				 						@else
											...
											<div class="clear-fix"></div>
											<div class="text-right top">
												<a href="{{route('user.user_subscriptions.payment-zero' , ['user_subscription_id' => $user_subscription_details->id])}}" class="btn btn-danger" id="my_button">
...
<script>
    ...
    function applyCouponSubscription() {
    	...
    	var subscription_id = "{{$user_subscription_details->id}}";
    	...
    	$.ajax({
    		...
    		success : function(data) {
				...
				$("#remaining_amount").text("{{$user_subscription_details->amount}}");
