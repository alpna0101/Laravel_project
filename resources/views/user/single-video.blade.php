...
.video_ryt_new .thumb-class, .video_ryt_new .gif_like {
...
.gif_commnt_new .gif_inner.gif_commt {
   right: 0px;
left: unset;
margin: 0 auto;
    margin-bottom: 0px;
...
.gif_commnt_new .gif_outer.gif_like.gif_commt_btn {
    margin-top: 20px;
}
.gif_commnt_new .gif_outer.gif_like.gif_commt_btn button#comment_btn {
    max-width: 200px;
    width: 100%;
}
.btc {
...
<div class="y-content">
  @if (Auth::check())
<div id="btcmodel" class="modal fade" role="dialog">
 ...
 <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">BTC  Detail</h4>
      </div>
      ...
      <div class="modal-body">
       @if(@$btc)
                     ...
                     @else
                      <h4 class="modal-title">No Detail Found</h4>
                     @endif
      </div>
                  ...
                  <div class="single-video-sec">
                     ...
                     @if($video->product_id)
                        <div class="product_link">
                           <a href="{{url('/product_preview').'/'.$video->product_id}}" target="_blank">
                              <img src="{{url('/images/marketplace.png')}}" height="30" />
                           ...
                           <div class="video-title">
                              <div class="title row">
                                 <div class="col-lg-12 col-md-12 col-sm-12 col-lg-12 zero-padding">
                                    ...
                                    <div class="pull-right relative video_ryt_new">
                                       @if (Auth::check())
                                       <a   data-toggle="modal" href="#btcmodel"> <img src="{{asset('images/bit.png')}}" height="40" width="40"></a>&nbsp;
                                        <a class="thumb-class" onclick="tipVideo({{$video->video_tape_id}})"><img src="{{asset('/tipme.png')}}" height="40" width="40"> &nbsp;<span id="tip_count">{{number_format_short($tip_count)}}</span></a>&nbsp;

<div class="gif_inner gift_like_gif"> <img src="{{asset('/tip.gif')}}"></div>
                                        <div class="gif_outer gif_like">
                                  ...
                                  <!--   <h4 class="video-desc">{{strip_tags($video->description)}}</h4> -->
                                    <hr>
                                 </div>
                                    ...
                                    <div id="comment_form">
                                       <div>
                                          <form method="post" id="comment_sent" name="comment_sent" action="{{route('user.add.comment')}}">
                                             ...
                                             <p class="underline"></p>
                                             <div class="gif_commnt_new">
                                             <div class="gif_inner gif_commt"> <img src="{{asset('/tip1.gif')}}"></div>
                                            <div class="gif_outer gif_like gif_commt_btn"> <button class="btn pull-right btn-sm btn-info btn-lg top-btn-space" type="submit" id="comment_btn">{{tr('comment')}}</button></div>
                                            </div>
                                             <div class="clearfix"></div>
                                          </form>
                                       </div>
...
<?php
   ...
   if(count($ads) > 0 && $ads != null) {
    ...
    if(isset($video->banner_title)){
   
       $title =  $video->banner_title;
    }
       if(isset($video->banner_title)){
          $des =  strip_tags($video->banner_description);
       }
   ?>
...
<script type="text/javascript">
   ...
   jQuery(document).ready(function(){ 
       ...
       playerInstance.setup({
           ...
           title: "{{$title}}",
           description:<?php echo json_encode($des); ?>,
           width: "100%",
           aspectratio: "16:9",
           primary: "flash",
           controls : true,
     ...
     playerInstance.on('ready', function(){
                        ...
                        var datetime = "Now: " + dt.getDate() + "/"
                                        + (dt.getMonth()+1)  + "/" 
                                        + dt.getFullYear() + " @ "  
                                        + dt.getHours() + ":"  
                                        + dt.getMinutes() + ":" 
                                        + dt.getSeconds();
                        // $(".jw-title-primary").append("<p style='position: absolute;right: 8px;top: 8px;'>"+datetime+"</p>")
                        });
     <?php if($title!="" or $des!=""){ ?>
             playerInstance.on('play', function(){
                            $(".jw-title").css("bottom","40px");
                        $(".jw-title").css("display","block");
                        $(".jw-title").css("top","inherit");
                        $(".jw-title").css("height","auto");
                        $(".jw-title").css("padding-right","0")
                        // $(".jw-title-primary").css("background","orangered")
                        $(".jw-title-primary").css("padding-right","170px;")
                        $(".jw-title-secondary").css("white-space","normal")
                        $(".jw-title-secondary").css("text-overflow","ellipsis")

                        // $(".jw-title-secondary").css("background","#fff")
                        // $(".jw-title-secondary").css("color","#000")
   ...
   function copyTextToClipboard() {
    ...
    function giftimage(){
       $('.gift_like_gif').fadeToggle("slow");
        setInterval(function() {
      giftimagehide();
    }, 2000);
    }
     function giftimagehide(){
       $('.gift_like_gif').css("display","none");
    }
   ...
   function likeVideo(video_id) {
      ...
      function tipVideo(video_id) {
		  @if(Auth::check())
      @if(Auth::user()->id == @$video->get_channel->user_id)
            alert("You  can not add tip credit to your own video");
       @else
        @if (Auth::user()->total_points>0)
       $.ajax({
           url : "{{route('user.video.tip')}}",
           data : {video_tape_id : video_id},
           type: "post",
           success : function(data) {
               if (data.success==true) {
   
                   $("#tip_count").html(data.tip_count);
                   $(".points-btn").html(data.total_points+" TipMe");
       ...
       @else
       alert("You  do'nt have sufficiant credit");
       @endif
           @endif
		   @endif
   }
