...
function total_revenue() {
    ...
    $user_subscriptions = UserSubscriptionPayment::sum('paid_amount');

    return $video_payments + $user_payments + $user_subscriptions;
}
...
function total_user_subscription_revenue($user_subscription_id = "") {

    if($user_subscription_id) {

        return UserSubscriptionPayment::where('user_subscription_id' , $user_subscription_id)->sum('paid_amount');
    }

    return UserSubscriptionPayment::sum('paid_amount');
}
...
function getAmountBasedChannel($id) {
    ...
    $user_subscription_amount = UserSubscriptionPayment::where('channel_id', $id)->where('status' , APPROVED)->sum('paid_amount');
    ...
    $amount = $ppv_amount+$ad_amount+$user_subscription_amount;
