...
class UserController extends Controller {
    ...
    public function broadcast(Request $request) {
        if($request->channel_id==""){
            $channl = Channel::where('user_id',Auth::user()->id)->first();
            if($channl){
               
             $request->request->add([ 
           
            
            'channel_id'=>$channl->id
           ]);  
           }else{
              return back()->with('flash_error', "You need to create a channel first");
           }     
         }
    ...
    public function index(Request $request) {
        ...
        $username = config('database.connections.mysql.username');
            Log::info(Setting::get('installation_process'));
        if($database && $username && Setting::get('installation_process') == 2) {
            ...
            $sponsored  = $this->UserAPI->sponsored_videos($request)->getData(); 
        ...
        if(Auth::check()) {
         ...
         $given_token = 0;
     $total_token =  DB::table('user_tokens')->select(DB::raw("SUM(token) as token"))->first();
     $given_token = $total_token->token;

      $token_left = config('app.available_token')-$given_token;
        ...
        $subscription = $query->get();
            ...
            return view('user.index')
                        ...
                        ->with('given_token', $given_token)

                        ->with('token_left', $token_left)
    ...
    public function training(Request $request) {
        ...
        $trending = $this->UserAPI->training_list($request)->getData();

        return view('user.training')->with('page', 'training')
                                    ->with('videos',$trending);
    
    }
      public function invoice_new(Request $request) {
        ...
        return view('user.invoice_new')->with('page', 'invoice')->with('subPage', 'invoice')->with('model', $model)->with('subscription',$subscription)
            ->with('model',$model);
    }
 public function topplayer(){
         $top =    DB::table('users as u')
        ->select('u.total_points as points','u.picture', 'u.id','u.name', \DB::raw("(select COUNT('v.id') AS video_count from video_tapes where user_id=u.id) as video_count,(select SUM(watch_count) from video_tapes as vi inner join users as ur on ur.id=vi.user_id where vi.user_id=u.id) as video_view_count,(select count(lv.id) from like_dislike_videos as lv inner join video_tapes as v on v.id=lv.video_tape_id where v.user_id=u.id and like_status='1') as video_like_count, (select sum(up.points) AS points_counts from video_tapes as v inner join user_points as up on up.video_tape_id=v.id  where up.owner_user_id = v.user_id and up.status = '1') as points_counts"))
        ...
        ->get();
       // print_r($top);
       return view('user.top')->with('page', 'top')
                                    ->with('top',$top);
        
    }
     public function addbtc(){
     
      return view('user.account.bitcoin');

     }
      public function editbtc(){
     $btc = UserBitcoin::where('user_id', Auth::user()->id)->first();
      return view('user.account.bitcoin')->with('btc',$btc);

     }
      public function btc_show(){
        $btc = UserBitcoin::where('user_id', Auth::user()->id)->first();
      return view('user.account.showbtc')->with('btc',$btc);
    }
     //  public function btc_show1($id){
     
     //  return view('user.account.bitcoin');

     // }
   public function btc_save(Request $request){
   	   if($request->id!=''){
          $btc =  UserBitcoin::find($request->id);
   	   }else{
   	   	$btc = new UserBitcoin;
   	   }
   	    
   	    $btc->btc_address = $request->btc_address;
   	    $btc->user_id = Auth::user()->id;

   	        if($request->hasFile('qr_code')){

                 $get = $request->file("qr_code");

                $image = $btc->QR_fileUpload($get);
        ...
        $btc->save();
        return redirect(route('user.profile.showbtc'));
    ...
    public function wishlist(Request $request) {
        ...
        $products =  DB::table('products')->leftjoin('product_ratings','product_ratings.product_id','=','products.id')->leftjoin('users','users.id','=','products.user_id')->leftjoin('product_wishlists','product_wishlists.product_id','=','products.id')
         ->select('products.*','product_wishlists.wish',DB::raw('AVG(product_ratings.rating) as ratings_average'),'users.name as username')->where('products.generated_by','U')->where('product_wishlists.user_id',Auth::user()->id)->where('product_wishlists.wish',true)->orderBy('id','desc')->groupBy('products.id')->paginate(20);
         // print_r($products);die;
        return view('user.account.wishlist')
                    ->with('page' , 'wishlist')
                    ->with('subPage' , 'user-wishlist')
                    ->with('videos' , $videos)->with('products' , $products);
    ...
    public function single_video(Request $request) {
        ...
        if ($data->success) {
            ...
            $btc="";
			if (Auth::check()){
           $btc = UserBitcoin::where('user_id', $response->video->get_channel->user_id)->first();
			}
            return view('user.single-video')
                        ...
                        ->with('tip_count',@$response->tip_count)
                        ...
                        ->with('banner_title', $response->banner_title)
                        ->with('banner_description', $response->banner_description)
                        ->with('btc', $btc);
   ...
   $given_token = 0;
     ...
     $given_token = $total_token->token;
       ...
       $token_left = config('app.available_token')-$given_token;
        return view('user.account.subscriptions')->with('subscriptions', $model)->with('page', 'Profile')->with('subPage', 'Subscriptions')->with('token_left', $token_left)->with('given_token', $given_token);
    }
    ...
    public function tipVideo(Request $request)  {
        $request->request->add([
            'id' => Auth::user()->id,
            'token'=>Auth::user()->token
        ]);
        
       $response = $this->UserAPI->Tipme($request);
    ...
    public function coin_payment(Request $request) {
        ...
        $response = $this->UserAPI->coin_payment($request)->getData();
    ...
    public function subscription_payment(Request $request) {
        ...
        if($request->payment_type == 1) {
            ...
            return redirect(route('user.card.coin_payment' , ['subscription_id' => $request->s_id, 'coupon_code'=>$request->coupon_code]));
        }
    }
 ...
 public function check_mail(Request $request) {
    ...
    */
    ...
    public function makingMoney(Request $request) {
        ...
        $videos = $this->UserAPI->makingMoney($request)->getData();

        return view('user.makingmoney')->with('page', 'making_money')
                                    ->with('videos',$videos);
