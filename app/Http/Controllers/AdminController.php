...
class AdminController extends Controller {
    ...
    public function dashboard() {
      ...
      $admin = Admin::first();
       ...
       $product_payment = DB::table('carts')
                ->join('orders', 'orders.id', '=', 'carts.order_id')
                  ->join('transactions', 'orders.id', '=', 'transactions.order_id')
                  ->join('products', 'products.id', '=', 'carts.product_id')->select("transactions.total_price as price")->where('orders.payment_status',true)->where('products.generated_by','A')->groupBy("orders.id")->orderBy("orders.id",'desc')->get();
                  $sum=0;
      foreach($product_payment as $key=>$value){
  
           $sum += $value->price;
         }
                 // print_r($product_payment);die;
...
$total_revenue =    $total_revenue+$sum;
   ...
   public function user_password(Request $request) {
        ...
        if($user) {
            ...
            return view('admin.users.password')->withUser($user)->with('sub_page','view-user')->with('page' , 'users');
    ...
    public function users_save(Request $request) {
        ...
        if($validator->fails()) {
            ...
            $user->coin_payment_pay_name = $request->coin_payment_pay_name ?: '';

            $user->mac_address = $request->mac_address ?: '';

            $user->gold_access_app_username = $request->gold_access_app_username ?: '';

            $user->gold_access_app_password = $request->gold_access_app_password ?: '';
            
            $user->media_box_voucher_code = $request->media_box_voucher_code ?: '';
 ...
 public function users_password(Request $request) {
            ...
            if($validator->fails()) {
             $error_messages = implode(',', $validator->messages()->all());
              return back()->with('flash_errors', $error_messages)->withInput();
                ...
                $subject = tr('user_change_password' , Setting::get('site_name'));

                $page = "emails.admin_user_password";
                ...
                Helper::send_email($page,$subject,$email,$email_data);
                if($user) {
                return redirect(route('admin.users', $user->id))->with('flash_success', "Password reset successfully");
               }else{
                return back()->with('flash_error', tr('admin_not_error')->withInput());
                }
            }
    ...
    */
     ...
     *
     */
    public function users_subscription_paln_save($s_id, $u_id) {
    ...
    public function videos_create(Request $request) {
        ...
        $product_list = DB::table('products')->whereIn('type',['product','service'])->where('status',1)->get();
  ...
  public function coin_price($id = "") {

        

        $data =  DB::table('coin_prices')->first();
        return view('admin.users.user_coin')
                ->withPage('user_coin')
                ->with('sub_page' , 'user_coin')
                ->with('data' , $data);
    
    }
    public function coin_price_update($id = "") {
  $data =  DB::table('coin_prices')->first();

       
        return view('admin.users.user_coin_update')
                ->withPage('user_coin_update')
                ->with('sub_page' , 'user_coin_update')
                ->with('data' , $data);
    
    }
        public function edit_coin_price(Request $request) {

 DB::table('coin_prices')->where('id', $request->id)->update(['price' => $request->price]);
  $data =  DB::table('coin_prices')->first();
        return view('admin.users.user_coin')
                ->withPage('user_coin')
                ->with('sub_page' , 'user_coin')
    ...
    */
    public function revenues() {
        ...
        $total_user_subscription_payments = UserSubscriptionPayment::sum('paid_amount');
        ...
        return view('admin.payments.revenues')
                        ...
                        ->with('total_user_subscription_payments', $total_user_subscription_payments)
                        ->withPage('payments')
        ...
        if($settings) {
            ...
            foreach ($settings as $setting) {
                ...
                if($setting->key == 'site_name') {
                    ...
                    $request_admin_ppv_commission = $request->admin_ppv_commission ?: 0;

                    $setting->value = $request_admin_ppv_commission < 100 ? $request_admin_ppv_commission : 100;

                    $user_ppv_commission = $request_admin_ppv_commission < 100 ? 100 - $request_admin_ppv_commission : 0;
                    ...
                    $request_referral_commission = $request->referral_commission ?: 0;

                    $setting->value = $request_referral_commission < 100 ? $request_referral_commission : 100;

                } else if($setting->key == "referral_commission2") {

                    $request_referral_commission2 = $request->referral_commission2 ?: 0;

                    $setting->value = $request_referral_commission2 < 100 ? $request_referral_commission2 : 100;
    ...
    */
    ...
    /**
     * Function Name: users_referrals()
     *
     * @uses get the referrals details for selected user
     *
     * @created vithya R
     *
     * @updated vithya R
    ...
    public function users_referrals(Request $request) {
        ...
        $user_referrer_details = UserReferrer::where('user_id', $request->id)->first();
        ...
        $referrals = Referral::where('parent_user_id', $request->id)->orderBy('created_at', 'desc')->paginate(12);
        ...
        return view('admin.users.referrals.index')->with('page', 'users')
                ->with('sub_page', 'view-user')
                ->with('user_details', $user_details)
    ...
    /**
     * Function Name: users_referrals_view()
     *
    ...
    public function users_referrals_view(Request $request) {
        ...
        return view('admin.users.referrals.view')
                    ->with('page', 'users')
                    ->with('sub_page', 'view-user')
                    ->with('user_details', $user_details)
                    ->with('user_referrer_details', $user_referrer_details)
     ...
     *
     * @uses To list out user_subscription details
     *
     * @created Anjana H
     *
     * @updated Anjana H
    ...
    public function user_subscriptions_index(Request $request) {
        ...
        $page = "user_subscriptions"; $sub_page = "user_subscriptions-view";
        ...
        if($request->channel_id) {
            ...
            $page = "channels"; $sub_page = "view-channels";

        }

        $user_subscriptions = $base_query->get();

        foreach ($user_subscriptions as $key => $user_subscription_details) {

            $user_subscription_details->channel_name = $user_subscription_details->channelDetails ? $user_subscription_details->channelDetails->name : "";

            $user_subscription_details->channel_username = $user_subscription_details->channelUserDetails ? $user_subscription_details->channelUserDetails->name : "";
        }

        return view('admin.user_subscriptions.index')
                    ->withPage($page)
                    ->with('sub_page', $sub_page)             
    ...
    /**
     * Function Name: user_subscription_create()
     *
     * @uses To create user_subscription details 
     *
     * @created Anjana H
     *
     * @updated Anjana H
    ...
    public function user_subscriptions_create(Request $request) {
        ...
        $channels = Channel::where('status', DEFAULT_TRUE)->orderBy('name' , 'asc')->get();

        foreach ($channels as $key => $channel_details) {

            $channel_details->is_selected = $channel_details->id == $request->channel_id ? YES : NO;

            $channel_details->username = $channel_details->getUser ? $channel_details->getUser->name : "";
        }

        return view('admin.user_subscriptions.create')
                    ->with('page' , 'user_subscriptions')
                    ->with('sub_page','user_subscriptions-create')
                    ->with('user_subscription_details', $user_subscription_details)
                    ->with('channels', $channels);
    }

    /**
     * Function Name: user_subscription_edit()
     *
     * @uses To create user_subscription details 
     *
     * @created Anjana H
    ...
    public function user_subscriptions_edit(Request $request) {
        ...
        $user_subscription_details = UserSubscription::where('id' ,$request->user_subscription_id)->first();

        $channels = Channel::where('status', DEFAULT_TRUE)->orderBy('name' , 'asc')->get();

        foreach ($channels as $key => $channel_details) {

            $channel_details->is_selected = $channel_details->id == $user_subscription_details->channel_id ? YES : NO;

            $channel_details->username = $channel_details->getUser ? $channel_details->getUser->name : "";
        }

        return view('admin.user_subscriptions.edit')
                    ->with('sub_page','user_subscriptions-view')
                    ->with('page' , 'user_subscriptions ')
                    ->with('user_subscription_details', $user_subscription_details)   
                    ->with('channels', $channels);
    ...
    public function user_subscriptions_save(Request $request) {
        ...
        try {            
            ...
            $validator = validator($request->all(), [
                ...
                'user_subscription_id'=> $request->user_subscription_id ? 'exists:user_subscriptions,id' : '',
                ]);
            ...
            if($request->user_subscription_id != '' ) {
                ...
                $user_subscription_details->update($request->all());
                ...
                if( $user_subscription_details->save()) {
                    ...
                    throw new Exception(tr('admin_user_subscription_save_error'), 101);
                }
            ...
            $message = $request->user_subscriptions_id ? tr('admin_user_subscription_update_success') : tr('admin_user_subscription_create_success'); 

            return redirect()->route('admin.user_subscriptions.view', ['user_subscription_id' => $user_subscription_details->id] )->with('flash_success',$message);       
    ...
    /**
     * Function Name: user_subscriptions_view()
     *
     * @uses To view the user_subscription details based on user_subscription id
     *
     * @created Anjana H
     *
     * @edited Anjana H
    ...
    public function user_subscriptions_view(Request $request) {
        ...
        try {
            ...
            if (count($user_subscription_details) == 0) {

                throw new Exception(tr('admin_user_subscription_not_found'), 101);
            }

            return view('admin.user_subscriptions.view')
                        ->withPage('user_subscriptions')
                        ->with('sub_page','user_subscriptions-view')
                        ->with('user_subscription_details' , $user_subscription_details);
     ...
     *
     * @uses To delete the user subscription details based on selected user subscription id
     *
     * @created vidhya R
     *
     * @updated 
     *
     * @param integer $user_subscription_id
     * 
     * @return response of success/failure details
     *
     */
    public function  user_subscriptions_delete(Request $request) {
        ...
        try {
            ...
            if ($user_subscription_details->delete()) {
                ...
                return back()->with('flash_success',tr('admin_user_subscription_delete_success')); 

            } else {

                throw new Exception(tr('admin_user_subscription_delete_error'));
            ...
            return redirect()->route('admin.user_subscriptions.index')->with('flash_error', $error);
    ...
    public function user_subscriptions_status(Request $request) {
        ...
        try {
            ...
            $user_subscription_details->status = $user_subscription_details->status ? DECLINED : APPROVED;

            if( $user_subscription_details->save()){

                DB::commit();

                $message = $user_subscription_details->status ? tr('admin_user_subscription_approve_success') : tr('admin_user_subscription_decline_success');

                return redirect()->back()->with('flash_success', $message);

            } else{

                throw new Exception(tr('admin_user_subscription_status_error'));
    ...
    /**
     * Function Name: user_subscriptions_payments()
     *
     * @uses To list out user_subscription payments
     *
     * @created vidhya R
     *
     * @updated
     ...
     * @return view page 
     */
    public function user_subscriptions_payments(Request $request) {

        $base_query = UserSubscriptionPayment::orderBy('created_at','desc');

        // initialize variables

        $channel_details = $user_subscription_details = $user_details = [];

        $view_page = "admin.user_subscriptions.payments.index";

        if($request->user_subscription_id) {

            $user_subscription_details = UserSubscription::find($request->user_subscription_id);

            $base_query = $base_query->where('user_subscription_id', $request->user_subscription_id);

            $view_page = "admin.user_subscriptions.payments.user_subscription_based";
        ...
        if($request->channel_id) {
            ...
            $view_page = "admin.user_subscriptions.payments.channel_based";
        ...
        if($request->user_id) {
            ...
            $view_page = "admin.user_subscriptions.payments.user_based";


        }

        $user_subscription_payments = $base_query->get();
        ...
        if($request->page) {
            ...
            $page = "payments"; $sub_page = "user_subscriptions-payments";
    ...
    public function save_product(Request $request,$product_id = null){
         ...
         $old_main_image = $request->get("old_image");

         if($old_main_image){
            $product->image = $old_main_image;
         }
     ...
     if($delete_image){
        ...
        if(count($ex) > 0){
            ProductImage::whereIn("id",$ex)->delete();
             ...
             return redirect(route('admin.products'))->with('success', 'Product ${success_text} successfully');
    ...
    public function productList(){
        ...
        $myproduct = Product::where("generated_by","A")->paginate(20);
        return view('admin.product.my_product')
                    ->with('page', '')
                    ->with('products',$myproduct);
    }

    /*edit product*/
    public function editProduct($product_id){

        $product = Product::with("getImages")->find($product_id);
       
       return view('admin.product.add_product') ->with('sub_page','')
                    ->with('page' , '')->with('data',$product); 
   }
    ...
    public function sponsored_status_change($id) {
        $video = VideoTape::find($id);
        $video->isSponsored = $video->isSponsored ? DEFAULT_FALSE : DEFAULT_TRUE;

        $video->save();

        if($video->isSponsored == DEFAULT_TRUE) {

            $message = tr('admin_video_sponsored');

        } else {

            $message = tr('admin_video_not_sponsored');
    ...
    public function owner_save(Request $request) {

        $owner = new OwnerPage;
             if($request->hasFile('picture')){

                $get = $request->file("picture");

                $image = $owner->owner_fileUpload($get);
        ...
        $owner->save();
         ...
         return back()->with('flash_success',"Owner added successfully");
        ...
        //     $getOwner = OwnerPage::orderBy('order','DESC')->first();
        //     if($getOwner) {
        //         $order = $getOwner->order + 1;
        //     } else {
        //         $order = 0;
        //     }
        ...
        //         $owner->image = Helper::normal_upload_picture($request->file('picture'), "/uploads/owners/");
        ...
        //         // $user->save();
        //         $message = tr('admin_add_owner');

        //         return redirect(route('admin.pages.owners', $user->id))->with('flash_success', $message);
    ...
    public function owner_status_change(Request $request){

        $owner_status = OwnerPage::find($request->id);

        if($owner_status){

            $owner_status->status = $request->status;

            $owner_status->save();

            if($request->status==DEFAULT_FALSE){

                return back()->with('flash_success',tr('owner_inactive_success'));

            } else{

                return back()->with('flash_success',tr('owner_active_success'));

            }

        } else {

            return back()->with('flash_error',tr('owner_id_not_found'));
        }

    }

    public function owner_status_delete(Request $request){

        $owner_status = OwnerPage::find($request->id);

        if($owner_status){

            Helper::delete_picture($owner_status->image, "/uploads/owners/");

            if($owner_status->delete()) {

                return back()->with('flash_success',tr('owner_delete_success'));

            } else{

                return back()->with('flash_success',tr('admin_not_error'));
     ...
     public function customerorders(){
          ...
          if(@$_GET['action']=="ongoing"){
             $orders = DB::table('carts')
                ->join('orders', 'orders.id', '=', 'carts.order_id')
                  ->join('transactions', 'orders.id', '=', 'transactions.order_id')
                  ->join('products', 'products.id', '=', 'carts.product_id')->select('orders.*','transactions.*')->where('generated_by',"A")->where('orders.current_status',"IP")->where('products.type','!=','seller_token')->groupBy("orders.id")->orderBy("orders.id",'desc')->get();
             ...
             $orders = DB::table('carts')
                ->join('orders', 'orders.id', '=', 'carts.order_id')
                  ->join('transactions', 'orders.id', '=', 'transactions.order_id')
                  ->join('products', 'products.id', '=', 'carts.product_id')->select('orders.*','transactions.*')->where('generated_by',"A")->where('orders.current_status',"D")->where('products.type','!=','seller_token')->groupBy("orders.id")->orderBy("orders.id",'desc')->get();
                  foreach($orders as $key=>$order){
                   $orders[$key]->products = DB::table('carts')->join('products', 'products.id', '=', 'carts.product_id')->leftjoin('users', 'users.id', '=', 'products.user_id')->select('products.*','carts.quantity',"users.name as username")->where('carts.order_id',$order->order_id)->groupBy("products.id")->get();
         ...
         $orders = DB::table('carts')
                ->join('orders', 'orders.id', '=', 'carts.order_id')
                  ->join('transactions', 'orders.id', '=', 'transactions.order_id')
                  ->join('products', 'products.id', '=', 'carts.product_id')->select('orders.*','transactions.*')->where('generated_by',"A")->where('orders.current_status',"P")->where('products.type','!=','seller_token')->groupBy("orders.id")->orderBy("orders.id",'desc')->get();
                  foreach($orders as $key=>$order){
                   $orders[$key]->products = DB::table('carts')->join('products', 'products.id', '=', 'carts.product_id')->leftjoin('users', 'users.id', '=', 'products.user_id')->select('products.*','carts.quantity',"users.name as username")->where('carts.order_id',$order->order_id)->groupBy("products.id")->get();
        ...
        return view('admin.product.customer_orders')->with('orders',$orders);
    }
          ...
          $order = Order::find($orders->order_id);
           ...
           $order->save();
            $notification =  new Notification;
                    ...
                    $notification->message =   "Admin dispatched your order view order listing for shipping detail";
                    $notification->save();

            return redirect()->route('admin.customerorders')->with('flash_message' , 'Shipping Info has been added successfully');
        }
     }
                ...
                ->join('products', 'products.id', '=', 'carts.product_id')->select('products.name','products.id','products.image','products.user_id','products.price','products.description','carts.quantity')->where('orders.id',$orders->order_id)->get();
                  ...
                  $address = UserAddress::find($orders->addres_id);
                     ...
                     $email_data['transaction'] = $transaction;
                       ...
                       return view('admin.product.view_invoice')->with('email_data',$email_data);
      }

       public function point_create() {

        $data = new PointSetting;
        $point_type = PointType::all();
         
        return view('admin.point_setting.create')->with('page' , 'point_setting')
                   
                    ->with('data', $data)->with('point_type',$point_type);
    }
      public function point_edit($id) {

        $data =PointSetting::find($id);
        $point_type = PointType::all();
         
        return view('admin.point_setting.edit')->with('page' , 'point_setting')
                   
                    ->with('data', $data)->with('point_type',$point_type);
    }
     public function save_point(Request $req,$id = null){
        if(@$req->id !=""){
             $point = PointSetting::find($req->id);
        }else{
             $point = new PointSetting;
        }
      
       $point->type = $req->type;
       $point->point_type_id = $req->point_type_id;
       $point->point = $req->point;
       $point->save();
        return redirect()->route('admin.point_index')->with('flash_message' , 'Point  has been added successfully');

     }
       public function point_index() {
          $data =  DB::table('point_settings')->join('point_types', 'point_types.id', '=', 'point_settings.point_type_id')->select("point_settings.*","point_types.name")->get();
          // print_r($data);die;
          return view('admin.point_setting.index')->with('page' , 'point_setting')
                   ->with('data', $data);

       }
 public function delete_point($id) {
     $point = PointSetting::find($id);
     $point->delete();
     return redirect()->route('admin.point_index')->with('flash_message' , 'Point  has been deleted successfully');
 }
