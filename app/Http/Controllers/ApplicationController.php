...
class ApplicationController extends Controller {
    ...
    public function search_video(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            $products = Product::where('name','like', '%'.$q.'%')->orderBy('id' , 'desc')->get();
             if($products) {
                ...
                foreach ($products as $i => $key) {
                    ...
                    if($check <=10) {
                        ...
                        array_push($items,['label'=>$key->name,'category'=>"Products"]);

                    } if($check == 10 ) {

                        array_push($items,['label'=>"View All" ,'category'=>"Products"]);
