                ...
                if ($subscription) {
                    ...
                    $is_coupon_applied = COUPON_NOT_APPLIED;
                           ...
                           if ($request->coupon_code) {
                                    ...
                                    // Create user applied coupon
                                    if($check_coupon->code == 2002) {
                                    $user_coupon = UserCoupon::where('user_id', $user->id)->where('coupon_code', $request->coupon_code)
                                    ->first();
                                    // If user coupon not exists, create a new row
                                    if ($user_coupon) {
                                    if ($user_coupon->no_of_times_used < $coupon->per_users_limit) {
...
function subsciption_update_payment_status(){
       ...
       if ($data1['result']['status'] == 100){
                ...
                if($user_payment->subscription_amount == "0.99"){
                    $user_token->token = 1;
                    $user_token->user_id = $id;
                    $token = $user_token->save();
                }elseif($user_payment->subscription_amount == "99"){
                    $user_token->token = 3;
                    $user_token->user_id = $id;
                    $token = $user_token->save();
                }elseif($user_payment->subscription_amount== "299"){
                    $user_token->token = 5;
                    $user_token->user_id = $id;
                    $token = $user_token->save();
                }
