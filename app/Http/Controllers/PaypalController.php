...
class PaypalController extends Controller {
  ...
  public function order_pay($id){
    ...
    public function getPaymentStatus(Request $request) {
        ...
        if ($result->getState() == 'approved') { // payment made
            ...
            if($user_payment_details) {
                ...
                $user_payment_details->amount = $paid_amount = $user_payment_details->subscription_amount - $user_payment_details->coupon_amount;
                ...
                $referral_amount = $paid_amount * $referral_commission;
                $user_payment_details->referral_commission = $referral_amount ?: "";

                Log::info("Amount ".$user_payment_details->amount);

                $referral_amount2 = $paid_amount * $referral_commission2;
                $user_payment_details->referral_commission2 = $referral_amount2 ?: "";


                $user_payment_details->save();

                PaymentRepo::referral_amount_update($user_payment_details->user_id, $user_payment_details->referral_commission);
                PaymentRepo::tier2_referral_amount_update($user_payment_details->user_id, $user_payment_details->referral_commission2);
                 PaymentRepo::referral_token_update($user_payment_details->user_id);
    ...
    /**
     *
     * Function name: user_subscriptions_payment_by_paypal
     *
     * @uses used create paypal redirection for user channel subscriptions
     *
     * @created Vidhya R
     *
     * @updated Vidhya R
    ...
    public function user_subscriptions_payment_by_paypal(Request $request) {
        ...
        $user_details = User::find($request->logged_user_id);

        if(count($user_subscription_details) == 0 || !$user_details) {
        ...
        if ($request->coupon_code) {
            ...
            if ($coupon) {
                ...
                if ($coupon->status == COUPON_INACTIVE) {
                    ...
                    $check_coupon = $this->UserAPI->check_coupon_applicable_to_user($user_details, $coupon)->getData();
                    ...
                    if ($check_coupon->success) {
                        ...
                        if ($coupon->amount_type == PERCENTAGE) {
                            ...
                            $amount_convertion = amount_convertion($coupon->amount, $user_subscription_details->amount);

                        }

                        if ($amount_convertion < $user_subscription_details->amount  && $amount_convertion > 0) {

                            $total = $user_subscription_details->amount - $amount_convertion;
        ...
        $redirect_urls = new RedirectUrls();
        $redirect_urls->setReturnUrl(route('user.user_subscriptions.payment-paypal-status'))
                    ->setCancelUrl(url('/'));
        ...
        Session::put('user_channel_subscription_payment_id', $payment->getId());

        Session::put('user_subscription_id', $request->user_subscription_id);

        if(isset($redirect_url)) {

            $user_subscription_payment = new UserSubscriptionPayment;

            $user_subscription_payment->payment_id  = $payment->getId();

            $user_subscription_payment->user_id = $request->logged_user_id;
            ...
            $user_subscription_payment->payment_mode = PAYPAL;

            $user_subscription_payment->paid_amount = 0;
            ...
            $user_subscription_payment->expiry_date = date('Y-m-d H:i:s');

            // $user_subscription_payment->coupon_amount = $coupon_amount;

            // $user_subscription_payment->coupon_code = $request->coupon_code ? $request->coupon_code : "";

            // $user_subscription_payment->is_coupon_applied = $is_coupon_applied;

            // $user_subscription_payment->coupon_reason = $is_coupon_applied ? $coupon_reason : '';
    ...
    /**
     *
     * Function name: user_subscriptions_payment_paypal_status
     *
     * @uses used create paypal redirection for user channel subscriptions
     *
     * @created Vidhya R
     ...
     * 
     */
    public function user_subscriptions_payment_paypal_status(Request $request) {

        Log::info("user_subscriptions_payment_paypal_status");

        // Get the payment ID before session clear

        $user_channel_subscription_payment_id = Session::get('user_channel_subscription_payment_id');

        $user_subscription_id = Session::get('user_subscription_id');
        
        // clear the session payment ID
     
        if (empty($request->paymentId) || empty($request->token) || empty($user_channel_subscription_payment_id)) {
            
            $error_message = tr('payment_id_session');

            $user_subscription_id = Session::get('user_subscription_id');

            PaymentRepo::user_subscription_payment_failure_save(Auth::user()->id, $user_subscription_id , $error_message , "");

            Session::forget('user_subscription_id');

            Session::forget('user_channel_subscription_payment_id');
        ...
        try { 
            ...
            $payment = Payment::get($user_channel_subscription_payment_id, $this->_api_context);
       ...
       // echo '<pre>';print_r($result);echo '</pre>';exit; // DEBUG RESULT, remove it later
        ...
        if ($result->getState() == 'approved') { // payment made
            ...
            Log::info("user_channel_subscription_payment_id".$user_channel_subscription_payment_id);

            $user_subscription_payment_details = UserSubscriptionPayment::where('payment_id', $user_channel_subscription_payment_id)->first();

            $user_subscription_details = UserSubscription::find($user_subscription_id);

            // Check the PPV and video details

            if(count($user_subscription_payment_details) == 0 || count($user_subscription_details) == 0) {

                $error_message = tr('user_subscription_details_not_found');
            ...
            if(!$user_subscription_payment_details) {

                $user_subscription_payment_details = new UserSubscriptionPayment;

                Log::info("Create new user_subscription_payment_details");

            }

            $check_previous_payment = UserSubscriptionPayment::where('user_id' , $user_subscription_payment_details->user_id)
                                        ->where('channel_id', $user_subscription_details->channel_id)
                                        ->where('status', PAID_STATUS)
                                        ->orderBy('created_at', 'desc')
                                        ->first();

            Log::info("check_previous_payment".print_r($check_previous_payment, true));

            $user_subscription_payment_details->expiry_date = date('Y-m-d H:i:s',strtotime("+{$user_subscription_details->plan} months"));

            if($check_previous_payment) {

                if (strtotime($check_previous_payment->expiry_date) >= strtotime(date('Y-m-d H:i:s'))) {

                    $user_subscription_payment_details->expiry_date = date('Y-m-d H:i:s', strtotime("+{$user_subscription_details->plan} months", strtotime($check_previous_payment->expiry_date)));
            ...
            $user_subscription_payment_details->payment_id  = $user_channel_subscription_payment_id;

            $user_subscription_payment_details->user_id = Auth::user()->id;

            $user_subscription_payment_details->user_subscription_id = $user_subscription_details->id;

            $user_subscription_payment_details->channel_id = $user_subscription_details->channel_id;

            $user_subscription_payment_details->paid_amount = $total = $user_subscription_details->amount;

            $user_subscription_payment_details->payment_mode = PAYPAL;

            $user_subscription_payment_details->subscription_amount = $user_subscription_details->amount;

            $user_subscription_payment_details->subscription_plan = $user_subscription_details->plan;

            $user_subscription_payment_details->status = PAID_STATUS;
            ...
            $user_subscription_payment_details->admin_subscribe_amount = $admin_amount;

            $user_subscription_payment_details->user_subscribe_amount = $user_amount;
            ...
            return redirect()->route('user.user_subscriptions.payment-success',$user_subscription_details->id)->with('flash_success', $payment->message);
