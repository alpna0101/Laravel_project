...
class UserController extends Controller {
 ...
 public function topplayer(){
        ...
        ->select('u.total_points as points','u.picture', 'u.id','u.name', \DB::raw("(select COUNT('v.id') AS video_count from video_tapes where user_id=u.id) as video_count,(select count(vi.id) from viewers as vi inner join video_tapes as v on v.id=vi.video_id where v.user_id=u.id) as video_view_count,(select count(lv.id) from like_dislike_videos as lv inner join video_tapes as v on v.id=lv.video_tape_id where v.user_id=u.id and like_status='1') as video_like_count"))
