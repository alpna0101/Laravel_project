...
class UserController extends Controller {
 ...
 public function topplayer(){
        ...
        ->select('u.total_points as points','u.picture', 'u.id','u.name', \DB::raw("(select COUNT('v.id') AS video_count from video_tapes where user_id=u.id) as video_count,(select count(vi.id) from viewers as vi inner join video_tapes as v on v.id=vi.video_id where v.user_id=u.id) as video_view_count,(select count(lv.id) from like_dislike_videos as lv inner join video_tapes as v on v.id=lv.video_tape_id where v.user_id=u.id and like_status='1') as video_like_count, (select sum(up.points) AS points_counts from video_tapes as v inner join user_points as up on up.video_tape_id=v.id  where up.owner_user_id = v.user_id and up.status = '1') as points_counts"))
        
        ->orderBy('video_like_count', 'desc')
        ->groupBy('u.id')
        ->take(20)
        ->get();
       // print_r($top);
