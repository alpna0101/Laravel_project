...
class CommonRepository {
    ...
    public static function video_save(Request $request) {
        ...
        try {
            ...
            if($validator->fails()) {
                ...
                if(@$request->is_sponsored && $request->is_sponsored == '1') {
                    $model->isSponsored = 1;
                } else {
                    $model->isSponsored = 0;
                }

                if(@$request->product_id && $request->product_id != '0' && $request->product_id != '' && $request->product_id != 0) {
                ...
                if (in_array($request->video_type , [ VIDEO_TYPE_YOUTUBE , VIDEO_TYPE_OTHERS] )) {
                    ...
                    $uploadVideovalidator = Validator::make( $request->all(), array(
                        'video'=>$request->id ? 'mimes:mp4,mov,ogg,qt' : 'required|mimes:mp4,mov,ogg,qt',
                    ));
                ...
                $model->banner_title = $request->has('banner_title') ? $request->banner_title : $model->banner_title;
                  $model->banner_description = $request->has('banner_description') ? $request->banner_description : $model->banner_description;
                ...
                $is_training = $request->has('is_training') ? $request->is_training : $model->is_training;
                  // echo $model->is_training;die;
    ...
    public static function upload_video_image($request) {
        ...
        try {
            ...
            if ($video) {
                ...
                $video->banner_title = $request->has('banner_title') ? $request->banner_title : $video->banner_title;
                  $video->banner_description = $request->has('banner_description') ? $request->banner_description : $video->banner_description;
                ...
                $video->product_id = $request->has('product_id') ? ($request->product_id == '0' || $request->product_id == 0) ? NULL : $request->product_id  : $video->product_id;
    ...
    /**
     * Function Name : save_subscription_plan
     *
     ...
     *
     */
    public static function save_subscription_plan($s_id, $u_id) {
     ...
     *
     */
    // public static function user_subscriptions_save($request) {

        // try {

            // $user_subscription_details = UserSubscription::find($request->user_subscription_id);

            // $load = UserSubscriptionPayment::where('user_id', $u_id)->orderBy('created_at', 'desc')->first();

            // $user_subsciption_payment_details = new UserSubscriptionPayment();

            // $user_subsciption_payment_details->user_subscription_id = $request->user_subscription_id;

            // $user_subsciption_payment_details->channel_id = $request->channel_id;

            // $user_subsciption_payment_details->amount = $request->amount ? $request->channel_id : $user_subscription_details->amount ;

            // $user_subsciption_payment_details->subscription_amount =  $user_subscription_details->amount;

            // $user_subsciption_payment_details->payment_id = ($user_subsciption_payment_details->amount > 0) ? uniqid(str_replace(' ', '-', 'PAY')) : 'Free Plan'; 

            // if ($load) {
            //     $user_subsciption_payment_details->expiry_date = date('Y-m-d H:i:s', strtotime("+{$user_subscription_details->plan} months", strtotime($load->expiry_date)));
            // } else {
            //     $user_subsciption_payment_details->expiry_date = date('Y-m-d H:i:s',strtotime("+{$user_subscription_details->plan} months"));
            // }



            // $user_subsciption_payment_details->status = DEFAULT_TRUE;

            // if ($user_subsciption_payment_details->save())  {

            //     $user_subsciption_payment_details->user->user_type = DEFAULT_TRUE;

            //     if($user_subsciption_payment_details->amount == 0) {

            //         $user_subsciption_payment_details->user->zero_subscription_status = DEFAULT_TRUE;
            //     }

            //     if ($user_subsciption_payment_details->user->save()) {
